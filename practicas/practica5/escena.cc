...
Escena::Escena(char **argv){
    ...
    std::vector<float> dirf = { 0.0, 0.0, 20.0, 0.0 }; // (x,y,z,w) 

    miIluminacion = new Iluminacion(caf, cdf, csf, dirf, 0);
    miIluminacion2= new Iluminacion(caf, cdf, csf, dirf, 1);
/*
...
void Escena::draw_objects() {
    ...
    switch(toupper(tecla)){
        ...
        case '4':
            ...
            break;
        case '5':
            miCamion->setGrado(grado1,grado2,grado3,grado4, grado5, grado8);
...
int Escena::teclaPulsada(unsigned char Tecla1,int x,int y){
    ...
    //Tecla1 = toupper(Tecla1);
        switch(Tecla1){
            ...
            case 'x':
                rotacionCamara=+100;
                miIluminacion->rotacionCamara(rotacionCamara, foco);
                cout<<"Camara"<<rotacionCamara<<endl;
                break;
            case 'X':
                rotacionCamara=(-100);
                miIluminacion->rotacionCamara(rotacionCamara, foco);
...
switch (Tecla1){
    ...
    case GLUT_KEY_F1:if(onOFF == true){
                    miIluminacion->apaga(0);
                    onOFF=false;
                }else{
                miIluminacion->enciende(0);
                onOFF=true;
            }
        break;
    case GLUT_KEY_F2:if(onOFF == true){
                    miIluminacion2->apaga(1);
                    onOFF=false;
                }else{
                miIluminacion2->enciende(1);
                onOFF=true;
            }
        break;

    //case GLUT_KEY_F2:miIluminacion->enciende(0);;break;
    case GLUT_KEY_F3:miIluminacion->enciendeFocos();break;
    case GLUT_KEY_F4:miIluminacion->rotacion();break;
